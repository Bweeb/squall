...
describe Squall::NetworkZone do
  ...
  describe "#show" do
    ...
    it "returns not found for invalid network zone id" do
      expect { @network_zone.show(404) }.to raise_error(Faraday::Error::ResourceNotFound)
    end
  ...
  describe "#edit" do
    ...
    it "raises an error for an invalid network zone id" do
      expect { @network_zone.edit(404, @valid) }.to raise_error(Faraday::Error::ResourceNotFound)
    end
  end
  ...
  describe "#delete" do
    ...
    it "returns NotFound for invalid network zone id" do
      expect { @network_zone.delete(404) }.to raise_error(Faraday::Error::ResourceNotFound)
    end
  end
  ...
  describe "#attach" do
    ...
    it "returns NotFound error for invalid id" do
      expect { @network_zone.attach(404, 1) }.to raise_error(Faraday::Error::ResourceNotFound)
    end
    
    it "returns NotFound error for invalid network id" do
      expect { @network_zone.attach(1, 404) }.to raise_error(Faraday::Error::ResourceNotFound)
    end
  ...
  describe "#detach" do
    ...
    it "returns NotFound error for invalid id" do
      expect { @network_zone.detach(404, 1) }.to raise_error(Faraday::Error::ResourceNotFound)
    end
    
    it "returns NotFound error for invalid network id" do
      expect { @network_zone.detach(1, 404) }.to raise_error(Faraday::Error::ResourceNotFound)
    end
